filename = '/Users/sanderarmus/Kool/EverybodyCodes/everybody_codes_e2024_q03_p3.txt'
        ...
        for c in range(len(matrix[r])):
            if (0 < r < len(matrix) - 1) and (0 < c < len(matrix[r]) - 1):
                if (((matrix[r][c] == depth) or matrix[r][c] ==depth+1)  and
                    ...
                    ((matrix[r][c + 1] == depth) or (matrix[r][c+1] ==depth+1))and
                    ((matrix[r + 1][c + 1] == depth) or (matrix[r+1][c +1] ==depth+1))and
                    ((matrix[r - 1][c -1] == depth) or (matrix[r-1][c -1] ==depth+1))and
                    ((matrix[r +1][c - 1] == depth) or (matrix[r +1][c-1] ==depth+1))and
                    ((matrix[r -1 ][c + 1] == depth) or (matrix[r-1][c+1] ==depth+1))):
                    matrix[r][c] = depth + 1
                    new_count_plus += 1
                    count+=1
